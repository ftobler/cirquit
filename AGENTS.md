...
test: build-wasm        # runs build-wasm first, then these lines
...
| `just dev`             | Dev server; rebuilds the wasm engine first.            |
...
| `just fmt`             | Autoformat both halves.                                |
| `just build-wasm`      | Rebuild `web/src/wasm/` after an engine change.        |
...
Changing anything in `engine/` requires `just build-wasm` (or `just dev`, which
does it) before the frontend sees it.
...
up without help; after an engine change run `just build-wasm` and let the
...
- Never edit `web/src/wasm/`; it is generated by `just build-wasm`.

...
export function OtherOptionsDialog() {
  ...
  // Settings push no undo entry, so a misclick here would quietly cost
  // someone their colours and timestep with nothing to take it back.
  const resetAll = () => {
    if (!window.confirm('Reset all options to their defaults?')) return;
  ...
  return (
    ...
    >
      {/* The sections flow into as many columns as the panel is wide (three on
          a desktop dialog, one on a phone), so the whole option set is one
          screenful instead of a long scroll. Each section is kept whole by
          `break-inside: avoid`. */}

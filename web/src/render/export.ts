/** Offscreen circuit rendering for Save As Image and the clipboard copy: the
 *  same `def.draw` calls the frame loop makes, replayed onto a canvas at the
 *  upstream print margins (ImageExporter.java:137-153, 202-214). The geometry
 *  math is pure and tested; the drawing needs a 2D context and is verified by
 *  hand, exactly like the live canvas. */

import type { SimEngine } from '../engine/simulator';
import { defFor } from '../model/registry';
import type { CircuitElement, DrawContext, SimSettings } from '../model/types';

...
describe('scope o-line fidelity', () => {
  ...
  it('resolves scope indices through file position, past an unmodelled element', () => {
    // `417` (unijunction) is a code upstream creates but this build does not
    // model, so it still takes the element-list slot and the scope's index 2 is
    // the second resistor, not the first.
    const parsed = parseCircuit(

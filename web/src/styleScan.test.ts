...
// CSS would mean a custom property per element for no benefit. Every other
// inline style must be a class in a real stylesheet, so this scan keeps the
// `style={{` rule enforced in CI.
const POSITIONER_FILES = [
  'ui/ContextMenu.tsx',
  'ui/ScopeMenu.tsx',
...
describe('inline style scan', () => {
  it('keeps style={{ out of tsx files except the popup positioners', () => {
    const offenders = walk(SRC_DIR).filter((path) => {
      const relative = path.slice(SRC_DIR.length);
      if (POSITIONER_FILES.includes(relative)) return false;

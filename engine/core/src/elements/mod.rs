...
pub fn build_element(spec: &ElementSpec) -> Option<Box<dyn Element>> {
    let e: Box<dyn Element> = match spec.kind.as_str() {
        ...
        "fuse" => Box::new(passive::Fuse::new(spec)),
        "lamp" => Box::new(passive::Lamp::new(spec)),
        "thermistor" => Box::new(passive::Thermistor::new(spec)),
        "potentiometer" => Box::new(passive::Potentiometer::new(spec)),
        "ldr" => Box::new(passive::Ldr::new(spec)),
        ...
        "varactor" => Box::new(semiconductor::Diode::new_varactor(spec)),
